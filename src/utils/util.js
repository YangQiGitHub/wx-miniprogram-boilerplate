const {
  isUndefined, isDefined, isString, isNumber, isArray, isObject, isFunction, isPhone,
  noop, hasOwn,
  getString, getNumber, getArray, getObject, getFunction
} = require('./base.js');
...
const ajax = (url, {
  ...
  wx.request(Object.assign(request, {
    success: ({data, statusCode}) => {
      ...
      showLog && console.log && console.log('[AJAX SUCCESS]', statusCode, typeof data === 'object' ? data : data.toString().substring(0, 100));
      ...
      // 其他情况，执行错误回调
      failToast && wx.showToast({ title: data.message || '获取数据出错', icon: 'none' })
      isFunction(fail) && fail(Object.assign({ statusCode }, data));
    },
    fail: ({error, errorMessage}) => {
      modalLoading && wx.hideLoading();
      showLog && console.log && console.log('[AJAX FAIL]', error, errorMessage);
...
const updataGlobalData = (key, value) => {
  ...
  // 校验 globalData
  if (!globalData) { return console.error('[$updateGlobalData] globalData Not Find!'); }
  // 校验: 操作字段
  if (!isString(key) || key === '') { return console.error('[$updateGlobalData] key 不能为空!'); }
  // 取出已有信息
...
const updataStorageData = (key, value) => {
  try {
    if (!isString(key) || key === '') { return console.error('[$updateStorageData] key 不能为空!'); }
    ...
    console.error(`[ERROR]: ${value ? 'UPDATE' : 'GET'} Storage ${key} : `, e.stack)
  }
};
